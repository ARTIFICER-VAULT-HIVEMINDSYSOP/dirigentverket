...
export function renderPlayArena(ride, play = {}) {
  if (!ride || !ride.ok) {
    ...
    return `<div class="rider-play is-empty rider-bit is-dry-run" data-rider-play data-bit="32" data-dry-run="1" role="status"
      data-freq-pip="${ride?.hedge?.freqPip ? '1' : '0'}" data-freq-saknas="${!ride || !ride.hedge || ride.hedge.saknas ? '1' : '0'}" data-freq-progress="${ride?.hedge?.freqProgress ? '1' : '0'}" data-hedge-ghost="0" data-hedge-band="0" data-hedge-mid-pip="0" data-hedge-fade="0" data-hedge-fade-in="0"
  ...
  const markHtml = marks
    .map((row) => {
      ...
      const cls = [
        ...
        .filter(Boolean)
        .join(' ');
      return `<div class="${cls}" ${railAttr}${hedgeRailAttr}${hedgeMidAttr}${hedgeFadeAttr}${hedgeFadeInAttr} data-top="${top}%" style="top:${top}%">
