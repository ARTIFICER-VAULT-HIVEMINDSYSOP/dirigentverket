...
import { newProjectDraft } from './store.js';
...
export function avvikelseClass(v) {
  if (v > 0) return 'neg';
  if (v < 0) return 'pos';
  return '';
}
...
export function currentProject(routeId, ctx) {
  const { state } = ctx;
  const id = routeId || state.selectedId;
  return state.projects.find((p) => p.id === id) || state.projects[0] || null;
}

export function renderProject(routeId, ctx) {
  const { state, persist } = ctx;
  const p = currentProject(routeId, ctx);
  if (!p) {
...
export function renderForm(existing, ctx) {
...
export function renderKalkyl(ctx) {
  const { state } = ctx;
  const port = portfolioMetrics(state.projects);
  ...
  return `
    <p class="page-lead">Portföljkalkylen räknas om live. Avvikelse och täckningsbidrag visas bara när både budget och kostnad är ifyllda och större än noll. Artificer AI för Tradingskolan finns under <a href="#/robot">Artificer AI</a>.</p>
    ${riskBanner}
    <div class="kalkyl-live">
...
export function renderSynergier(ctx) {
  const { state } = ctx;
  const findings = detectSynergies(state.projects);
